...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const serialized = channels.map(ch => {
      // For short_form: only show short videos; long_form: show all (channel type already classified)
      let videos = ch.videos
      if (type === 'short_form') {
        const shortVids = ch.videos.filter(v => {
          const s = durationToSeconds(v.duration)
          return s > 0 && s < 240   // under 4 min for shorts display
        })
        videos = shortVids.length > 0 ? shortVids : ch.videos  // fallback to all if none qualify
      }
