...
export default function VideoGrid({
  ...
  // React Query infinite query hook for videos
  const {
    data,
    fetchNextPage,
    hasNextPage,
    ...
    queryFn: async ({ pageParam = 1 }) => {
      ...
      const params = new URLSearchParams({
        channelType, search, page: String(pageParam), limit: '24',
        niches: niches.join(','),
      ...
      if (!r.ok) throw new Error('Fetch videos failed')
      return r.json()
    },
    initialPageParam: 1,
    getNextPageParam: (lastPage: any, allPages: any[]) => {
      const currentCount = allPages.reduce((acc, p) => acc + (p.videos?.length ?? 0), 0)
  ...
  const videos = data ? data.pages.flatMap(p => p.videos || []) : []
  const total = data?.pages[0]?.total ?? 0

  const applyFilters = (f: VideoFilterValues) => { setVf(f) }
  ...
  return (
    <div className="p-5 max-w-[1400px]">
      ...
      <div className="flex gap-2 mb-4 items-center">
        ...
        <NicheDropdown selected={niches} onChange={n => setNiches(n)} />
      ...
      <div className="flex items-center justify-between mb-5">
        <span className="text-sm text-gray-600 dark:text-gray-400 font-medium">
          {isLoading ? 'Loading...' : `${(videos?.length ?? 0).toLocaleString()} of ${(total ?? 0).toLocaleString()} videos`}
        </span>
        {/* Column quick select */}
          ...
          <div className={`grid gap-3 ${COL_CLASSES[vf.columns] || COL_CLASSES[4]}`}>
            {videos.map(v => (
              <VideoCard
                key={v.videoId}
                video={v}
                onSimilarTitles={(q) => { setSearch(q); window.scrollTo({top:0,behavior:'smooth'}) }}
                onSimilarNiche={(niche) => { setNiches([niche]); setSearch(''); window.scrollTo({top:0,behavior:'smooth'}) }}
              />
            ))}
          </div>
          {hasNextPage && (
            <VideoInfiniteScroll loading={isFetchingNextPage} onLoadMore={fetchNextPage} />
          )}
        </>
      )}
